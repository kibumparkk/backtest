"""
BTC EMA 30 돌파 전략 백테스트

전략 설명:
- 가격이 EMA 30 이상일 때 매수 (보유)
- 가격이 EMA 30 미만일 때 매도 후 현금 보유
- EMA(Exponential Moving Average)는 최근 가격에 더 많은 가중치를 부여
- 슬리피지 0.2% 반영
"""
...
# ccxt는 API에서 데이터를 가져올 때만 필요 (파일 사용 시 불필요)
...
class EMABacktest:
    """EMA 30 돌파 전략 백테스트 클래스"""

    def __init__(self, symbol='BTC_KRW', start_date='2018-01-01', end_date=None, ema_period=30, data_source='file'):
        """
        Args:
            symbol: 티커 심볼 (default: 'BTC_KRW')
            start_date: 백테스트 시작일
            end_date: 백테스트 종료일 (None이면 오늘까지)
            ema_period: EMA 기간 (default: 30)
    ...
    def fetch_data(self):
        ...
        if self.data_source == 'file':
            ...
            try:
                ...
                print("ccxt module not found. Please install it to use API data source.")
    ...
    def calculate_signals(self, slippage=0.002):
        ...
        # span 파라미터는 기간을 의미하며, adjust=False로 설정하여 표준 EMA 계산
        df['EMA'] = df['Close'].ewm(span=self.ema_period, adjust=False).mean()

        # 포지션 계산: 가격이 EMA 이상이면 1 (매수), 미만이면 0 (매도/현금)
    ...
    def calculate_performance_metrics(self):
        ...
        # NaN 값이 있는 초기 기간 제거 (EMA 계산을 위한 기간)
        df = df[df['EMA'].notna() & df['cumulative_return'].notna()]
    ...
    def plot_results(self, save_path='ema_30_backtest_results.png'):
        """결과 시각화"""
        df = self.results.copy()
        # NaN 값이 있는 초기 기간 제거 (EMA 계산을 위한 기간)
        df = df[df['EMA'].notna() & df['cumulative_return'].notna()]
        ...
        ax1.plot(df.index, df['EMA'], label=f'EMA {self.ema_period}', linewidth=1.5, alpha=0.8, color='orange')
        ...
        ax1.set_title(f'{self.symbol} - EMA {self.ema_period} Breakthrough Strategy', fontsize=14, fontweight='bold')
        ...
        ax2.plot(df.index, df['cumulative_return'], label='EMA Strategy', linewidth=2)
    ...
    def print_metrics(self, metrics):
        """성과 지표 출력"""
        print("\n" + "="*80)
        print(f"EMA {self.ema_period} 돌파 전략 백테스트 결과".center(80))
        print("="*80)
        print(f"\n기간: {self.start_date} ~ {self.end_date}")
        print(f"종목: {self.symbol}")
        print(f"EMA 기간: {self.ema_period}")
        print("\n" + "-"*80)
    ...
    def run(self, slippage=0.002, save_path='ema_30_backtest_results.png'):
        """백테스트 실행"""
        # 데이터 다운로드
        self.fetch_data()
...
def main():
    ...
    results.to_csv('ema_30_backtest_results.csv')
    print("Results saved to ema_30_backtest_results.csv")

    # EMA 기간 변화에 따른 성과 비교
    print("\n" + "="*80)
    print("EMA 기간 변화에 따른 성과 비교".center(80))
    print("="*80)

    ema_periods = [10, 20, 30, 50, 100]
    comparison_results = []

    for period in ema_periods:
        bt = EMABacktest(symbol='BTC_KRW', start_date='2018-01-01', ema_period=period, data_source='file')
        bt.fetch_data()
