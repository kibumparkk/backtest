"""
비트코인 멀티 타임프레임 전략 (LOOKAHEAD BIAS FIXED)
주봉 필터 + 일봉 타이밍으로 유의미한 성과 개선

벤치마크: Close > SMA30 (일봉 only, Sharpe 1.66)
목표: Sharpe 1.8+ 달성

FIX: 주봉 신호를 shift(1) 후 reindex하여 lookahead bias 제거
"""
...
class MultiTimeframeStrategyCorrected:
    """멀티 타임프레임 전략 백테스터 (Lookahead bias 수정)"""
    ...
    def load_data(self):
        """일봉 및 주봉 데이터 로드"""
        print("="*80)
        print("Loading Bitcoin data (CORRECTED VERSION - No Lookahead Bias)...")
        print("="*80)
        ...
        print("\n⚠️ FIX APPLIED: Weekly signals shifted by 1 period before reindex")
        print("   This ensures weekly signal is available AFTER week ends, not on Monday")
        print("="*80 + "\n")
    ...
    def strategy_weekly_sma20_daily_sma30(self):
        ...
        # FIX: Shift by 1 before reindex to avoid lookahead
    ...
    def strategy_weekly_golden_cross_daily_sma30(self):
        """주봉 SMA10 > SMA20 + 일봉 SMA30 위 - FIXED"""
        weekly = self.weekly_data.copy()
        weekly['SMA10'] = weekly['Close'].rolling(10).mean()
        weekly['SMA20'] = weekly['Close'].rolling(20).mean()
        ...
        # FIX: Shift by 1 before reindex
        weekly_signal_shifted = weekly['golden_cross'].shift(1)
    ...
    def strategy_weekly_macd_daily_sma30(self):
        """주봉 MACD > Signal + 일봉 SMA30 위 - FIXED"""
        weekly = self.weekly_data.copy()
        ...
        # FIX: Shift by 1 before reindex
        weekly_signal_shifted = weekly['macd_bullish'].shift(1)
        weekly_signal = weekly_signal_shifted.reindex(daily.index, method='ffill')
    ...
    def strategy_weekly_sma200_daily_sma30(self):
        ...
        # FIX: Shift by 1 before reindex
        weekly_signal_shifted = weekly['bull_market'].shift(1)
        weekly_signal = weekly_signal_shifted.reindex(daily.index, method='ffill')
    ...
    def strategy_weekly_dual_sma_daily_sma30(self):
        """주봉 SMA20 & SMA50 위 + 일봉 SMA30 위 - FIXED"""
        weekly = self.weekly_data.copy()
        weekly['SMA20'] = weekly['Close'].rolling(20).mean()
        weekly['SMA50'] = weekly['Close'].rolling(50).mean()
        ...
        # FIX: Shift by 1 before reindex
        weekly_signal_shifted = weekly['strong_trend'].shift(1)
        weekly_signal = weekly_signal_shifted.reindex(daily.index, method='ffill')
    ...
    def strategy_weekly_alignment_daily_sma30(self):
        """주봉 SMA10 > SMA20 > SMA50 정렬 + 일봉 SMA30 위 - FIXED"""
        weekly = self.weekly_data.copy()
        weekly['SMA10'] = weekly['Close'].rolling(10).mean()
        weekly['SMA20'] = weekly['Close'].rolling(20).mean()
        weekly['SMA50'] = weekly['Close'].rolling(50).mean()
        ...
        # FIX: Shift by 1 before reindex
        weekly_signal_shifted = weekly['aligned'].shift(1)
        weekly_signal = weekly_signal_shifted.reindex(daily.index, method='ffill')
    ...
    def strategy_weekly_donchian_daily_sma30(self):
        """주봉 20주 신고가 근처 + 일봉 SMA30 위 - FIXED"""
        weekly = self.weekly_data.copy()
        ...
        # FIX: Shift by 1 before reindex
        weekly_signal_shifted = weekly['near_high'].shift(1)
        weekly_signal = weekly_signal_shifted.reindex(daily.index, method='ffill')
    ...
    def run_all_strategies(self):
        """모든 전략 실행"""
        print("="*80)
        print("Running Multi-Timeframe Strategies (CORRECTED - No Lookahead Bias)")
        print("="*80 + "\n")
    ...
    def analyze_results(self, metrics_df):
        ...
        print("\n" + "="*120)
        print(f"{'Multi-Timeframe Strategy Results (CORRECTED)':^120}")
        print("="*120)
        ...
        if len(better) > 0:
            print("\n" + "-"*120)
            print("🎯 Top Strategies (Beating Benchmark - After Fixing Lookahead Bias):")
            print("-"*120)
...
def main():
    """메인 실행"""
    print("\n" + "="*80)
    print("Bitcoin Multi-Timeframe Strategy Analysis (CORRECTED)")
    print("Goal: Beat Daily SMA30 benchmark (Sharpe 1.66) with meaningful improvement")
    print("FIX: Weekly signals shifted by 1 period to eliminate lookahead bias")
    print("="*80)

    analyzer = MultiTimeframeStrategyCorrected(slippage=0.002)
    analyzer.load_data()
    ...
    # CSV 저장
    metrics_df.to_csv('bitcoin_mtf_corrected_results.csv', index=False)
    print(f"\n✓ Results saved to bitcoin_mtf_corrected_results.csv")
